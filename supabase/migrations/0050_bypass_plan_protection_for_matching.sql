-- El trigger restaurants_protect_plan_fields (protect_restaurant_plan_fields())
-- bloquea cualquier UPDATE a plan/plan_expires_at/is_partner/is_verified
-- salvo que auth.jwt() ->> 'email' sea el superadmin — pensado para que
-- nadie más edite esos campos por su cuenta. El problema: también bloqueaba
-- las funciones security definer de verificación automática de pagos
-- (0045-0048), porque el webhook (sin sesión) y el dueño del restaurante
-- reportando su propio pago (sesión suya, no de superadmin) tampoco pasan
-- ese chequeo. El pago quedaba "approved" pero el plan nunca se extendía
-- de verdad — el trigger revertía plan/plan_expires_at en silencio.
--
-- Arreglo: agrega una segunda condición de bypass, una bandera de
-- configuración LOCAL a la transacción (set_config(..., true) — se
-- resetea sola al terminar, no puede "quedarse pegada" ni afectar otras
-- operaciones). Solo las funciones de emparejamiento automático la
-- activan, justo antes de su propio UPDATE a restaurants.
...
as $function$
begin
  if coalesce(auth.jwt() ->> 'email', '') not in ('joseph.ro.silva@gmail.com')
     and coalesce(current_setting('app.bypass_plan_protection', true), '') <> 'on'
  then
    new.plan := old.plan;
...
begin
  ...
  if v_plan_id is not null then
    ...
    perform set_config('app.bypass_plan_protection', 'on', true);
