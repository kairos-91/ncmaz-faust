  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <StatusBadge status={payment.status as Status} />
      </div>
      ...
      <div className="mt-3 grid gap-1 text-sm text-neutral-700 dark:text-neutral-300">
        ...
        {payment.bank_paid_from && <p>Banco: {payment.bank_paid_from}</p>}
        {payment.payment_reference && (
          <p>Referencia: {payment.payment_reference}</p>
        )}
        {payment.amount_paid_bs && (
          <p>Monto pagado: Bs {payment.amount_paid_bs}</p>
        )}
        <p className="text-xs text-neutral-500 dark:text-neutral-500">
          {new Date(payment.created_at).toLocaleString("es-VE", {
            dateStyle: "medium",
            timeStyle: "short",
          })}
        </p>
