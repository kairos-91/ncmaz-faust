...
export async function createKitchenStaff(
  restaurantId: string,
  _prev: ActionState,
  formData: FormData,
): Promise<ActionState> {
...
export async function toggleKitchenStaffActive(
  restaurantId: string,
  staffId: string,
  isActive: boolean,
) {
...
export async function deleteKitchenStaff(restaurantId: string, staffId: string) {
  const supabase = await requireOwnedRestaurant(restaurantId);
  const { error } = await supabase
    .from("kitchen_staff")
    .delete()
