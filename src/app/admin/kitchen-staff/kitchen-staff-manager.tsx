...
import {
  createKitchenStaff,
  deleteKitchenStaff,
  toggleKitchenStaffActive,
} from "./actions";
import type { KitchenStaff } from "@/lib/supabase/database.types";
import { getDictionary, type Dictionary, type Locale } from "@/lib/i18n/dictionaries";

type T = Dictionary["kitchenStaffManager"];
...
export function KitchenStaffManager({
  ...
  const t = getDictionary(locale).kitchenStaffManager;
  const boundCreate = createKitchenStaff.bind(null, restaurantId);
    ...
    <li className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          onClick={() =>
            startTransition(() =>
              toggleKitchenStaffActive(restaurantId, member.id, !member.is_active),
            )
          }
          className={
            member.is_active
        ...
        <button
          ...
          onClick={() => {
            ...
            startTransition(() => deleteKitchenStaff(restaurantId, member.id));
          }}
        >
          {t.delete}
        </button>
